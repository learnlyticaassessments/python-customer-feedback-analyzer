...
def evaluate_student_code(student_id, student_file):
    ...
    workspace_dir = os.path.join(os.getcwd(), "student_workspace")
    ...
    subprocess.run(
        [sys.executable, "student_workspace/run.py"],
        capture_output=True,
        text=True
    )
...
def run_all():
    ...
    for _, row in df.iterrows():
        ...
        scp_command = f"scp -o StrictHostKeyChecking=no -i ~/.ssh/id_rsa ubuntu@{ip}:/home/ubuntu/Desktop/customer_feedback_analyzer_v2/student_workspace/solution.py {student_dir}/solution.py"
